---
title: "Age, time and the international correlation of cervical cancer (Version 006: full numbers)"
...
knitr::include_graphics("C:/Users/schultefrohlinder/Documents/IntCorr_Paper/graphs/data_sel_upd.jpg")
```
...
inc <- inc %>%
  ...
  summarise("Ni" = last(cumsum(N)),
            "Pi" = last(cumsum(P)),
            "IR" = last(cumsum(N))*100000/last(cumsum(P)))
...
incVIII <- dataVIII %>%
  ...
  mutate("Ni" = V5, # cases
         "Pi" = V6) %>% # personyears
  select(Ni, Pi) %>% # drop sex, age, cases, personyears
  mutate(age.grp = c("ag1", "ag1", "ag2", "ag2", "ag3", "ag3", "ag4", "ag4"))%>%  
  ungroup %>%
  group_by(age.grp) %>%
  summarise(IR = last(cumsum(Ni))*100000/last(cumsum(Pi)),
            Ni = last(cumsum(Ni)),
            Pi = last(cumsum(Pi))) %>%
  #mutate("ci5" = 8) %>%
...
glob_25_34 <- read.csv("C:/Users/schultefrohlinder/Documents/R/NatHistHPV/data sets/globocan_25_34.csv", 
                       row.names = NULL) %>%
  mutate(age.grp = "ag1")
glob_35_44 <- read.csv("C:/Users/schultefrohlinder/Documents/R/NatHistHPV/data sets/globocan_35_44.csv", 
                       row.names = NULL)%>%
  mutate(age.grp = "ag2")
glob_45_54 <- read.csv("C:/Users/schultefrohlinder/Documents/R/NatHistHPV/data sets/globocan_45_54.csv", 
                       row.names = NULL)%>%
  mutate(age.grp = "ag3")
glob_55_64 <- read.csv("C:/Users/schultefrohlinder/Documents/R/NatHistHPV/data sets/globocan_55_64.csv", 
                       row.names = NULL)%>%
  mutate(age.grp = "ag4")
...
inc_glob <- glob.data %>%
  select(country = row.names, IR = Crude.Rate., age.grp) %>% # shifted col names. crude rate is actually ASR 
  filter(country %in% info$country[is.na(info$REGISTRY)]) %>%
  mutate(Ni = IR, Pi = 100000) 
...
# corr.glm <- glm(Ni/Pi ~ prev + prev*age.grp -1, family = poisson(link = "identity"), data = inc.prev.upd, weights = Pi)
...
inc.prev.upd[inc.prev.upd$prev == 0, "prev"] <- 0.000001 # to provide correct starting values
```
...
inc.prev.upd[inc.prev.upd$prev == 0, "prev"] <- 0.000001 # to provide starting values

# one regression model per age group
for(i in levels(inc.prev.upd$age.grp)) {
  nd <- data.frame("prev" = inc.prev.upd[inc.prev.upd$age.grp == i, "prev"])  
  
  upd.glm[[i]] <- glm(Ni/Pi~ prev -1, family = poisson(link = "identity"), 
                                 data = inc.prev.upd[inc.prev.upd$age.grp == i,], weights = Pi)
  
  upd.coef[[i]] <- round(coef(upd.glm[[i]])[1]*100000, 2)
  
  upd.p[[i]] <- ifelse(coef(summary(upd.glm[[i]]))[,4] < 0.001, "<0.0001", ">0.0001" )
  
  upd.pred[[i]] <- data.frame("pred" = predict(upd.glm[[i]], nd, se.fit = TRUE)$fit*100000,
                            "ci_u" = predict(upd.glm[[i]], nd, se.fit = TRUE)$fit*100000 
                              + 1.96*(predict(upd.glm[[i]],nd, se.fit = TRUE)$se.fit*100000),
                            "ci_l" = predict(upd.glm[[i]], nd, se.fit = TRUE)$fit*100000 
                              - 1.96*(predict(upd.glm[[i]], nd, se.fit = TRUE)$se.fit*100000),
                            "age.grp" = i, "prev" = nd)
}

# to test for significant difference between age groups
upd.glm.all <- glm(Ni/Pi~ prev*age.grp, family = poisson(link = "identity"), 
                                 data = inc.prev.upd, weights = Pi)
```
```{r eval = F, echo = F}
# predictions for 100% HPV prevalence to compare 
# to age-specific transition rate from HPV to cervical cancer below
for(i in levels(inc.prev.upd$age.grp)) {
nd_100 <- data.frame("prev" = 100)  
  upd.100[[i]] <- data.frame("pred" = predict(upd.glm[[i]], nd_100, se.fit = TRUE)$fit*100000,
                            "ci_u" = predict(upd.glm[[i]], nd_100, se.fit = TRUE)$fit*100000 
                              + 1.96*(predict(upd.glm[[i]],nd_100, se.fit = TRUE)$se.fit*100000),
                            "ci_l" = predict(upd.glm[[i]], nd_100, se.fit = TRUE)$fit*100000 
                              - 1.96*(predict(upd.glm[[i]], nd_100, se.fit = TRUE)$se.fit*100000),
                            "age.grp" = i)
  }
upd.100
...
The estimated spearman rank coefficients and poisson slope coefficients are specified for the respective age group.
The black line represents the fitted Poisson model.  

```{r eval = TRUE, echo = FALSE, fig.align='center', fig.height=16, fig.width=20, out.width="500px"}
####VII. plots####

options(scipen=999) # turns scientific notation off
labels_upd <- c(`25-34` = str_c(" 25-34y
        correlation coef = ", round(spear$all$`25-34`$estimate, 2), " (p = " , round(spear$all$`25-34`$p.value, 5),
                            ")
      slope = ", upd.coef$`25-34`, " (p = ", upd.p$`25-34`, ")"),
            `35-44` = str_c(" 35-44y
        correlation coef = ", round(spear$all$`35-44`$estimate, 2), " (p = " , round(spear$all$`35-44`$p.value, 5), 
                            ")
      slope = ", upd.coef$`35-44`, " (p = ", upd.p$`35-44`, ")"),
            `45-54` = str_c(" 45-54y
        correlation coef = ", round(spear$all$`45-54`$estimate, 2), " (p = " , round(spear$all$`45-54`$p.value, 5),
                            ")
      slope = ", upd.coef$`45-54`, " (p = ", upd.p$`45-54`, ")"),
            `55-64` = str_c(" 55-64y 
        correlation coef = ", round(spear$all$`55-64`$estimate, 2), " (p = " , round(spear$all$`55-64`$p.value, 5),
                            ")
      slope = ", upd.coef$`55-64`, " (p = ", upd.p$`55-64`, ")"))
...
gg_loc <- ggplot2::ggplot(upd_dat, aes(x = prev, y = IR)) +
  theme_classic() +
  geom_point(aes(color = loc.prev, shape = loc.prev), size = 3) +
  scale_shape_manual(values = sh) +
  guides(color = guide_legend(ncol = 1),
         shape = guide_legend(ncol = 1)) +
  facet_wrap(~age.grp, labeller = labeller(age.grp = labels_upd), scales = "free_x") +
  theme(strip.text.x = element_text(size = 18)) +
  labs(x = "hrHPV prevalence (%)",
       y = "Cervical Cancer Incidence (per 100000 women)",
       color = "Location",
       shape = "Location") +
  #ylim(0, 160) +
  xlim(0, 36) +
  ...
  geom_ribbon(aes(x = prev, ymin = ci_l, ymax = ci_u), fill = "#969696", alpha = 0.5) +
  geom_line(aes(x = prev, y = pred), size = 1.2, col = "black")
...
```{r, eval = TRUE, echo = FALSE, out.width="500px"}
knitr::include_graphics("C:/Users/schultefrohlinder/Documents/IntCorr_Paper/graphs/mult_stage.jpg")
```
...
pooled.hrisk <- prev.data %>%
  ...
  filter(sga3 >= minp_tr &  sga3 < maxp_tr)
prvl <- pooled.hrisk %>%
  mutate(hpvpos = rowSums(pooled.hrisk[, Hrisk])) %>% # number of different hpv infections
  mutate(hpvsino = ifelse(hpvpos > 0, 1, 0)) %>% # factor if hpv  positive or negative for high risk types. NA now as 0 !?
  mutate(hpvsino = factor(hpvsino, levels = c(0,1), labels = c("neg", "pos"))) %>%
  mutate(age.grp = factor(cut(sga3, seq(minp_tr, maxp_tr, ageintp_tr), right = FALSE), labels = stringr::str_c("ag", 1:((maxp_tr-minp_tr)/ageintp_tr), collapse = NULL))) %>%
  ungroup() %>%
  group_by(sgcentre, age.grp) %>%
...
if(nbag < 9) {
  prvl.long$age <- c(((as.numeric(stringr::str_sub(prvl.long$age.grp, -1))-1)*ageint_trp + minp_tr) + 0:(ageintp_tr-1))
}else{
  prvl.long$age <- c((as.numeric(stringr::str_sub(prvl.long$age.grp, -2))*ageintp_tr - minp_tr) + 0:(ageintp_tr-1))
}
...
nbag <- length(levels(prvl$age.grp))
if(nbag < 9) {
  prvl.long$age <- c(((as.numeric(stringr::str_sub(prvl.long$age.grp, -1))-1)*ageintp_tr + minp_tr) + 0:(ageintp_tr-1))
}else{
  prvl.long$age <- c((as.numeric(stringr::str_sub(prvl.long$age.grp, -2))*ageintp_tr - minp_tr) + 0:(ageintp_tr-1))
}
prvl.long$Year <- as.numeric(prvl.long$stY) # to be able to join 
...
for(d in c(4, 9)){
  ...
  for(f in c(20, 25, 30, 35, 40, 45)) {
    ...
    DpC <- Dp %>%
      ...
      summarise(Npos = sum(round(Npos, 0)), 
                Nicc = sum(round(Nicc, 0)),
                prev = sum(round(Npos, 0))*100/sum(PopFemale),
                IR = sum(round(Nicc, 0))*100000/sum(PopFemale), # incidence rate in whole population
                logIR = log(sum(round(Nicc, 0))/sum(PopFemale)), 
                se = sqrt(sum(round(Nicc, 0))/sum(round(Npos, 0))), # standard error of rates = rate^2/cases = sqrt(cases)/personyears [Esteve p. 52]
                tr = sum(round(Nicc, 0))/sum(round(Npos, 0)), # transition rate
                log_tr = log(sum(round(Nicc, 0))/sum(round(Npos, 0))), # log transition rate
                selog = sqrt(1/sum(round(Npos, 0)))) %>% # var(log(k/m)) = 1/(Rate*pyears); se(log(k/m)) = sqrt(1/k) [Esteve p. 53]
    ...
    lcmm_out$tr_lcmm[[stringr::str_c(mina, "_", maxa, "_2")]] <-
     ...
     # ## no log...............
    lcmm_NAT$tr_lcmm[[stringr::str_c(mina, "_", maxa)]] <-
      tr_NAT <- lcmm(tr ~ YsncS * cov, random = ~ YsncS, subject = "Location",  ng = 1, data = DpC, link = "3-equi-splines")
    lcmm_NAT$tr_lcmm[[stringr::str_c(mina, "_", maxa, "_1")]] <-
      tr_NAT_1 <- lcmm(tr ~ YsncS, random = ~ YsncS, subject = "Location",  ng = 1, data = DpC[DpC$cov == "yes", ], link = "3-equi-splines")
    lcmm_NAT$tr_lcmm[[stringr::str_c(mina, "_", maxa, "_2")]] <-
      tr_NAT_2 <- lcmm(tr ~ YsncS, random = ~ YsncS, subject = "Location",  ng = 1, data = DpC[DpC$cov == "no", ], link = "3-equi-splines")
    ...
    #no log
    lcmm_NAT$tr_pred[[stringr::str_c(mina, "_", maxa, "_1")]] <- 
      pred_NAT_1 <- predictY(tr_NAT_1, nd, methInteg = 1, draws = TRUE)
    lcmm_NAT$tr_pred[[stringr::str_c(mina, "_", maxa, "_2")]] <- 
      pred_NAT_2 <- predictY(tr_NAT_2, nd, methInteg = 1, draws = TRUE)
    ...
    lcmm_out$tr_lambda[[stringr::str_c(mina, "_", maxa, "_2")]] <- 
      tr_lambda_2 <- data.frame("YsncS" = tr_pred_2$times[2:15,]-0.5, 
        ...
        #no log
    lcmm_NAT$lambda_NAT[[stringr::str_c(mina, "_", maxa, "_1")]] <- 
      lambda_NAT_1 <- data.frame("YsncS" = pred_NAT_1$times[2:15,]-0.5, 
                              "lambda" = diff(pred_NAT_1$pred[, 1]*1000)) # to rescale to per 1000 women
    lcmm_NAT$lambda_NAT[[stringr::str_c(mina, "_", maxa, "_2")]] <- 
      lambda_NAT_2 <- data.frame("YsncS" = pred_NAT_2$times[2:15,]-0.5, 
                              "lambda" = diff(pred_NAT_2$pred[, 1]*1000))
  }
}
```
...
#### Figure 4: Transition rate to cervical cancer over time since high-risk HPV detection by age and screening coverage estimated using ecological data (log scale)    
``` {r eval = TRUE, echo = FALSE}
#### 12. Transition Plots ####

#### Joint transition plot, seperate models by screening ####

ticks <- c(0.1, 0.5, 2, 5, 10, 20)
gg_12 <- ggplot(DpC_12, aes(x = YsncS, y = log_tr)) +
  ...
  labs(title = "Transition rate from HPV to cervical cancer by age and screening",
       y = "Incidence Rate in hrHPV+ women (per 1000) on log scale", 
  ...
  scale_y_continuous(breaks= log(ticks/1000),  # natural scale
                     labels= ticks)+ 
...
\newpage 
#### Figure 6: The change in cervical cancer transition rate over time since high-risk HPV detection by screening coverage (log scale)
```{r echo = FALSE, eval = TRUE}
...
gg_lambda <- ggplot(df_lambda, aes(YsncS, lambda)) +
  ...
  labs(title = "Change in cervical cancer transition rate by screening coverage",
       ...
       y = "Change in cervical cancer transition rate (log scale)",
...
\newpage
Using age groups as continuous variable and screening coverage as categorical variable yielded a very significant (p<0.0001) effect of age on the transition rate and its trend over time since HPV detection as well as weakly significant (p<0.1) interaction terms between screening, age and the time since HPV detection, while keeping in mind that there were only two countries in the screening group(see below). The latter is significant at p<0.05 when using 5 year age groups (data not shown).   
...
# 3. Conclusion  
In conclusion, we could confirm the strong international correlation between hr-HPV prevalence and cervical cancer incidence consistant in all age groups. The slope of the Poisson regression model rises significantly with age, suggesting that the change in cancer incidence rate per percent HPV prevalence rises with age.   
Secondly we could observe a very significant effect of time since hr-HPV detection and age at hr-HPV detection and a small and weakly significant effect of high screening coverage on the transition rate from hr-HPV to cervical cancer estimated in this ecological study. The time trend of the transition rate stabilizes in higher age groups and earlier in countries with high screening coverage.   
The results are consistant with previous results of *Sharma, M. et al. Using HPV prevalence to predict cervical cancer incidence. (Int. J. Cancer 2013)* and *Plummer, M. et al. Time since first sexual intercourse and the risk of cervical cancer. (Int J Cancer 2012)* while adding the aspect of time and estimating absolute numbers.
...
####Figure 7: International correlation between cervical cancer incidence and high-risk HPV prevalence by source of incidence data
...
gg_source <- ggplot(upd_dat, aes(x = prev, y = IR)) +
  theme_classic() +
  geom_point(aes(color = ci5), size = 3) +
...
####Figure 8: International correlation between cervical cancer incidence and high-risk HPV prevalence by source using CI5 data only
...
for(i in levels(inc.prev.upd$age.grp)) {
  nd_CI5 <- data.frame("prev" = inc.prev.upd[inc.prev.upd$age.grp == i & inc.prev.upd$ci5 == "CI5", "prev"])  
  
  upd.glm_CI5[[i]] <- upd.glm_CI5 <- glm(Ni/Pi ~ prev -1, family = poisson(link = "identity"), data = 
                                 inc.prev.upd[inc.prev.upd$age.grp == i & inc.prev.upd$ci5 == "CI5",], weights = Pi)
  
  upd.coef_CI5[[i]] <- round(coef(upd.glm_CI5[[i]])[1]*100000, 2)
  
  upd.p_CI5[[i]] <- ifelse(coef(summary(upd.glm_CI5[[i]]))[,4] < 0.001, "<0.0001", ">0.0001" )
  
  upd.pred_CI5[[i]] <- data.frame("pred" = predict(upd.glm_CI5, nd_CI5, se.fit = TRUE)$fit*100000,
                            "ci_u" = predict(upd.glm_CI5, nd_CI5, se.fit = TRUE)$fit*100000 
                              + 1.96*(predict(upd.glm_CI5, nd_CI5, se.fit = TRUE)$se.fit*100000),
                            "ci_l" = predict(upd.glm_CI5, nd_CI5, se.fit = TRUE)$fit*100000 
                              - 1.96*(predict(upd.glm_CI5, nd_CI5, se.fit = TRUE)$se.fit*100000),
                            "age.grp" = i, "prev" = nd_CI5)
  }
...
labels_CI5 <- c(`25-34` = str_c("25-34y
        correlation coefficient = ", round(spear$ci5$`25-34`$estimate, 2), " (p = " , round(spear$ci5$`25-34`$p.value, 5),
                            ")
      slope = ", upd.coef_CI5$`25-34`, " (p = ", upd.p_CI5$`25-34`, ")"),
            `35-44` = str_c("35-44y
        correlation coefficient = ", round(spear$ci5$`35-44`$estimate, 2), " (p = " , round(spear$ci5$`35-44`$p.value, 5), 
                            ")
      slope = ", upd.coef_CI5$`35-44`, " (p = ", upd.p_CI5$`35-44`, ")"),
            `45-54` = str_c("45-54y
        correlation coefficient = ", round(spear$ci5$`45-54`$estimate, 2), " (p = " , round(spear$ci5$`45-54`$p.value, 5),
                            ")
      slope = ", upd.coef_CI5$`45-54`, " (p = ", upd.p_CI5$`45-54`, ")"),
            `55-64` = str_c("55-64y 
        correlation coefficient = ", round(spear$ci5$`55-64`$estimate, 2), " (p = " , round(spear$ci5$`55-64`$p.value, 5),
                            ")
      slope = ", upd.coef_CI5$`55-64`, " (p = ", upd.p_CI5$`55-64`, ")"))
```
...
gg_loc_ci5 <- ggplot2::ggplot(upd_dat_CI5[upd_dat_CI5$ci5 == "CI5",], aes(x = prev, y = IR)) +
  ...
  labs(title = "International Correlation of HPV and Cervical Cancer",
       subtitle = "CI5 Incidence data only",
       x = "hrHPV prevalence (%)",
       y = "Cervical Cancer Incidence (per 100 000 women)",
       color = "Location",
       shape = "Location") +
  ...
  geom_ribbon(aes(x = prev, ymin = ci_l, ymax = ci_u), col = "#bababa", alpha = 0.4) +
  geom_line(aes(x = prev, y = pred), size = 1.5, col = "black")
...
####Figure 9: International correlation between cervical cancer incidence and high-risk HPV prevalence on a natural scale.


```{r echo = FALSE, eval = TRUE, fig.height=16, fig.width=20}
## prediction
pred_NAT_1 <- rbind(data.frame("tr_m" = lcmm_NAT$tr_pred$`20_24_1`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag1", "cov" = 1),
                data.frame("tr_m" = lcmm_NAT$tr_pred$`25_34_1`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag2", "cov" = 1),
                data.frame("tr_m" = lcmm_NAT$tr_pred$`35_44_1`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag3", "cov" = 1),
                data.frame("tr_m" = lcmm_NAT$tr_pred$`45_54_1`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag4", "cov" = 1))
pred_NAT_2 <- rbind(data.frame("tr_m" = lcmm_NAT$tr_pred$`20_24_2`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag1", "cov" = 2),
                data.frame("tr_m" = lcmm_NAT$tr_pred$`25_34_2`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag2", "cov" = 2),
                data.frame("tr_m" = lcmm_NAT$tr_pred$`35_44_2`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag3", "cov" = 2),
                data.frame("tr_m" = lcmm_NAT$tr_pred$`45_54_2`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag4", "cov" = 2))

DpC_df <- rbind(DpC20, DpC25, DpC35, DpC45)
labels <- c(`ag1` = "20-24", `ag2` = "25-34", `ag3` = "35-44", `ag4` = "45-54")

DpC_NAT_1 <- left_join(DpC_df, pred_NAT_1, by = c("YsncS", "cov", "ag"))
DpC_NAT_2 <- left_join(DpC_df, pred_NAT_2, by = c("YsncS", "cov", "ag"))
DpC_NAT_12 <- rbind(DpC_NAT_1, DpC_NAT_2)
DpC_NAT_12 <- DpC_NAT_12[!is.na(DpC_NAT_12$tr_m),]
DpC_NAT_12$cov <- factor(DpC_NAT_12$cov, levels = c(1, 2), labels = c("no", "yes"))
DpC_NAT_12$tr_1000 <- DpC_NAT_12$tr * 1000
DpC_NAT_12$tr_m_1000 <- DpC_NAT_12$tr_m * 1000 # rescale to per 1000 HPV+ women

gg_12_NAT <- ggplot(DpC_NAT_12, aes(x = YsncS, y = tr_1000)) +
  theme_minimal() +
  geom_line(aes(col = Location, linetype = cov), size = 0.6) +
  scale_color_manual(values = col) +
  scale_linetype_manual(values = c(1, 3), labels = c("low screening", "high screening")) +
  geom_line(aes(x = YsncS, y = tr_m_1000, linetype = cov), size = 2, color = "black") +
  facet_wrap(~ ag, labeller = labeller(ag = labels), nrow = 1) +
  theme(strip.text.x = element_text(size = 20, face = "bold")) +
  #geom_ribbon(aes(x = YsncS, ymin = logr_lo, ymax = logr_hi), fill = 'azure4', alpha = 0.4)+
  labs(title = "Transition rate from HPV to cervical cancer by age and screening",
       y = "Incidence Rate in hrHPV+ women (per 1000) on natural scale", 
       x = "Years since HPV detection",
       subtitle = "Mixed effects model using splines. Model fit over 14y.") +
  #ylim(0, 200)+
  xlim(0, 11) +
...
####Figure 10: The change in cervical cancer transition rate over time since high-risk HPV detection by screening coverage on the natural scale

```{r echo = FALSE, eval = TRUE, fig.height=16, fig.width=20}

# 5y age groups: BETTER RESULTS .............................................................
## screen
df_g_NAT <- data.frame(lcmm_NAT$lambda_NAT$`20_24_1`)
df_lambda_NAT_1 <- df_g_NAT %>%
  mutate("20_24" = lambda,
         "25_34" = lcmm_NAT$lambda$`25_34_1`[, 2],
         "35_44" = lcmm_NAT$lambda$`35_44_1`[, 2],
         "45_54" = lcmm_NAT$lambda$`45_54_1`[, 2]) %>%
  select(-lambda) %>%
  gather("AgeGroup", "lambda", 2:5)%>%
  mutate(screen = 1) 
df_lambda_NAT_1$AgeGroup <- factor(df_lambda_NAT_1$AgeGroup,  levels = c("20_24", "25_34","35_44", "45_54"),  
                             labels = c("20-24", "25-34", "35-44", "45-54"))
## no screen
df_g_NAT <- data.frame(lcmm_NAT$lambda_NAT$`20_24_2`)
df_lambda_NAT_2 <- df_g_NAT %>%
  mutate("20_24" = lambda,
         "25_34" = lcmm_NAT$lambda$`25_34_2`[, 2],
         "35_44" = lcmm_NAT$lambda$`35_44_2`[, 2],
         "45_54" = lcmm_NAT$lambda$`45_54_2`[, 2]) %>%
  select(-lambda) %>%
  gather("AgeGroup", "lambda", 2:5) %>%
  mutate(screen = 2)
df_lambda_NAT_2$AgeGroup <- factor(df_lambda_NAT_2$AgeGroup,  levels = c("20_24", "25_34","35_44", "45_54"),  
                             labels = c("20-24", "25-34", "35-44", "45-54"))

df_lambda_NAT <- rbind(df_lambda_NAT_1, df_lambda_NAT_2)
df_lambda_NAT$screen <- factor(df_lambda_NAT$screen, levels = c(1, 2), labels = c("low coverage", "high coverage"))
...
gg_lambda_NAT <- ggplot(df_lambda_NAT, aes(YsncS, lambda)) +
  theme_classic() +
  geom_smooth(aes(color = AgeGroup), size = 1, se = FALSE) +
  scale_color_manual(values = cols) +
  facet_wrap(~ screen, ncol = 2) +
  ...
  labs(title = "Change in cervical cancer transition rate by screening coverage",
       subtitle = "per woman tested hrHPV+ at a given age", 
       x = "Years since hrHPV detection", 
       y = "Change in cervical cancer transition rate (per 1000 women)",
       color = "Age at hrHPV detection",
       linetype = "Age at hrHPV detection") + 
  xlim(0, 14) +
  guides(color = guide_legend(ncol = 1, label.hjust = 1)) +
