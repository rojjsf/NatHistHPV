---
title: "Age, time and the international correlation of cervical cancer (Version 004)"
...
knitr::opts_chunk$set(cache = TRUE, echo = FALSE, message = FALSE, fig.show = TRUE, warning = FALSE,
...
for(i in levels(inc.prev.upd$age.grp)) {
  ...
  upd.pred[[i]] <- data.frame("pred" = predict(upd.glm, nd, se.fit = TRUE)$fit,
                            "ci_u" = predict(upd.glm, nd, se.fit = TRUE)$fit 
                              + 1.96*(predict(upd.glm,nd, se.fit = TRUE)$se.fit),
                            "ci_l" = predict(upd.glm, nd, se.fit = TRUE)$fit 
                              - 1.96*(predict(upd.glm, nd, se.fit = TRUE)$se.fit),
                            "age.grp" = i, "prev" = nd)
  }
```
...
lcmm_out <- list("tr_lcmm","tr_lcmm_1","tr_lcmm_2", 
                 ...
                 "tr_lambda")
Dp_out <- list()
lcmm_NAT <- list("tr_NAT","tr_NAT_1","tr_NAT_2", 
                  "predNAT_","pred_NAT_1","pred_NAT_2")
...
# 5 year age groups
DpC24 <- DpCout$DpC20_24 %>%
  mutate(ag = 1) 
DpC29 <- DpCout$DpC25_29 %>%
  mutate(ag = 2) 
DpC34 <- DpCout$DpC30_34 %>%
  mutate(ag = 3)
DpC39 <- DpCout$DpC35_39 %>%
  mutate(ag = 4)
DpC44 <- DpCout$DpC40_44 %>%
  mutate(ag = 5)
DpC49 <- DpCout$DpC45_49 %>%
  mutate(ag = 6)
DpC_df_5 <- rbind(DpC24, DpC29, DpC34, DpC39, DpC44, DpC49)
tr_all_5 <- lcmm(log_tr ~ YsncS * ag * cov, random = ~ YsncS, subject = "Location",  ng = 1, 
             data = DpC_df_5, link = "4-equi-splines")
tr_ag_5 <- lcmm(log_tr ~ YsncS * ag, random = ~ YsncS, subject = "Location",  ng = 1, 
             data = DpC_df_5, link = "4-equi-splines")
...
DpC40 <- DpCout$DpC40_49 %>%
  mutate(ag = 3)
DpC_df_10 <- rbind(DpC20, 
                   DpC30, 
                   DpC40)
tr_all_10<- lcmm(log_tr ~ YsncS * ag * cov, random = ~ YsncS, subject = "Location",  ng = 1, 
             data = DpC_df_10, link = "4-equi-splines")
#summary(tr_all_10)
tr_ag_10<- lcmm(log_tr ~ YsncS * ag, random = ~ YsncS, subject = "Location",  ng = 1, 
             data = DpC_df_10, link = "4-equi-splines")
#summary(tr_ag_10)
#tr_all_10$BIC
# cov = 1 for countries with low/unknown screening coverage
tr_no_10 <- lcmm(log_tr ~ YsncS * ag, random = ~ YsncS, subject = "Location",  ng = 1, 
             data = DpC_df_10[DpC_df_10$cov == 1, ], link = "4-equi-splines")

# cov = 2 for countries with high screening coverage (Italy, Netherlands)
tr_scr_10 <- lcmm(log_tr ~ YsncS * ag, random = ~ YsncS, subject = "Location",  ng = 1, 
            data = DpC_df_10[DpC_df_10$cov == 2, ], link = "4-equi-splines")
...
tr_lcmm_1 <- lcmm(log_tr ~ YsncS, random = ~ YsncS, subject = "Location",  ng = 1, 
             data = DpC[DpC$cov == 1, ], link = "4-equi-splines")
...
labels_10 <- c(`ag1` = "20-29", `ag2` = "30-39", `ag3` = "40-49")
...
DpC_df_5 <- rbind(DpC24, DpC29, DpC34, DpC39, DpC44, DpC49)
labels_5 <- c(`ag1` = "20-24", `ag2` = "25-29", `ag3` = "30-34", `ag4` = "35-39", `ag5` = "40-44", `ag6` = "45-49")
DpC_df_5$ag <- as.factor(DpC_df_5$ag)
DpC_df_5$cov <- factor(DpC_df_5$cov, levels = c(1, 2), labels = c("no", "yes"))
...
DpC_1 <- left_join(DpC_df_5, pred_1, by = c("YsncS", "cov", "ag"))
DpC_2 <- left_join(DpC_df_5, pred_2, by = c("YsncS", "cov", "ag"))
DpC_12_5 <- rbind(DpC_1, DpC_2)
DpC_12_5 <- DpC_12_5[!is.na(DpC_12_5$tr_m),]

```
  
    
  
#### Figure 4: Transition rate to cervical cancer over time since high-risk HPV detection by age and screening coverage estimated using ecological data. Stratification in 10-year age groups.      
``` {r eval = TRUE, echo = FALSE}
#### 12. Transition Plots ####
...
gg_12_10 <- ggplot(DpC_12, aes(x = YsncS, y = log_tr)) +
  theme_minimal() +
  ...
  facet_wrap(~ ag, labeller = labeller(ag = labels_10)) +
...
\newpage 
#### Figure 5: Transition rate to cervical cancer over time since high-risk HPV detection by age and screening coverage estimated using ecological data. Stratification in 5-year age groups.   
``` {r eval = TRUE, echo = FALSE}
gg_12_5 <- ggplot(DpC_12_5, aes(x = YsncS, y = log_tr)) +
  theme_minimal() +
  geom_line(aes(col = Location, linetype = cov), size = 0.6) +
  scale_color_manual(values = col) +
  scale_linetype_manual(values = c(1, 3), labels = c("low screening", "high screening")) +
  geom_line(aes(x = YsncS, y = tr_m, linetype = cov), size = 2, color = "black") +
  facet_wrap(~ ag, labeller = labeller(ag = labels_5)) +
  theme(strip.text.x = element_text(size = 20, face = "bold")) +
  #geom_ribbon(aes(x = YsncS, ymin = logr_lo, ymax = logr_hi), fill = 'azure4', alpha = 0.4)+
  labs(title = "Transition rate from HPV to cervical cancer by age and screening",
       y = "log Incidence Rate in hrHPV+ women", 
...
\newpage
Using 10 year age groups as continuous variable and screening coverage as categorical variable yielded a very significant (p<0.0001) effect of age on the transition rate and its trend over time since HPV detection as well as significant (p<0.05) interaction terms between screening, age and the time since HPV detection.     
However no significant difference in intercept between the screening groups was found, while keeping in mind that there were only two countries in the screening group.  
When using 5-year age groups as covariate we found no significant effect of screening on the time trend of the transition rate. The interactions with age only and with time since HPV detection remained significant and thus screening can be considered an effect modifier of the transition rate from hr-HPV to cervical cancer (see below).    
Not including screening as variable yields an almost equivalent BIC for 5- and 10 year age groups *(anova is not possible with the lcmm package)*. The model including screening was thus considered preferable. 
```{r, eval = TRUE, echo = TRUE, dev.args=FALSE, results= FALSE}
...
list(c(tr_all_5$BIC, tr_ag_5$BIC), c(tr_all_10$BIC, tr_ag_10$BIC))
summary(tr_all_10) # 10-year age groups
summary(tr_all_5) # 5-year age groups
...
Secondly we could observe a significant effect of time since hr-HPV detection, age at hr-HPV detection and high screening coverage on the transition rate from hr-HPV to cervical cancer estimated in this ecological study. 
The time trend of the transition rate seems to stabilize in higher age groups, but earlier in countries with high screening coverage. 
...
for(i in levels(inc.prev.upd$age.grp)) {
  ...
  upd.pred_CI5[[i]] <- data.frame("pred" = predict(upd.glm_CI5, nd_CI5, se.fit = TRUE)$fit,
                            "ci_u" = predict(upd.glm_CI5, nd_CI5, se.fit = TRUE)$fit 
                              + 1.96*(predict(upd.glm_CI5, nd_CI5, se.fit = TRUE)$se.fit),
                            "ci_l" = predict(upd.glm_CI5, nd_CI5, se.fit = TRUE)$fit 
                              - 1.96*(predict(upd.glm_CI5, nd_CI5, se.fit = TRUE)$se.fit),
...
####Figure 9: International correlation between cervical cancer incidence and high-risk HPV prevalence by 10-year age group in natural scale   
The y-axis is cut at 0.02 for better visibility and thus Thailand(Songkla) is missing on the last graph.

```{r echo = FALSE, eval = TRUE, fig.height=16, fig.width=20}
## prediction
pred_NAT_1 <- rbind(data.frame("tr_m" = lcmm_NAT$tr_pred$`20_29_1`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag1", "cov" = 1),
                data.frame("tr_m" = lcmm_NAT$tr_pred$`30_39_1`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag2", "cov" = 1),
                data.frame("tr_m" = lcmm_NAT$tr_pred$`40_49_1`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag3", "cov" = 1))
pred_NAT_2 <- rbind(data.frame("tr_m" = lcmm_NAT$tr_pred$`20_29_2`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag1", "cov" = 2),
                data.frame("tr_m" = lcmm_NAT$tr_pred$`30_39_2`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag2", "cov" = 2),
                data.frame("tr_m" = lcmm_NAT$tr_pred$`40_49_2`$pred[,1],
                           "YsncS" = c(0:14), "ag" = "ag3", "cov" = 2))

pred_NAT_1$cov <- factor(pred_NAT_1$cov, levels = c(1, 2), labels = c("no", "yes"))
pred_NAT_2$cov <- factor(pred_NAT_2$cov, levels = c(1, 2), labels = c("no", "yes"))
...
gg_12_10_NAT <- ggplot(DpC_NAT_12, aes(x = YsncS, y = tr)) +
  theme_minimal() +
  geom_line(aes(col = Location, linetype = cov), size = 0.6) +
  scale_color_manual(values = col) +
  scale_linetype_manual(values = c(1, 3), labels = c("low screening", "high screening")) +
...
df_lambda_NAT_1 <- df_g_NAT %>%
  mutate("20_24" = lambda,
         "25_29" = lcmm_NAT$lambda$`25_29_1`[, 2],
         "30_34" = lcmm_NAT$lambda$`30_34_1`[, 2],
         "35_39" = lcmm_NAT$lambda$`35_39_1`[, 2],
         "40_44" = lcmm_NAT$lambda$`40_44_1`[, 2],
         "45_49" = lcmm_NAT$lambda$`45_49_1`[, 2]) %>%
  select(-lambda) %>%
  gather("AgeGroup", "lambda", 2:7)%>%
  mutate(screen = 1) 
df_lambda_NAT_1$AgeGroup <- factor(df_lambda_NAT_1$AgeGroup, levels = c("20_24", "25_29", "30_34", "35_39", "40_44", "45_49"),  
                             labels = c("20-24", "25-29", "30-34", "35-39", "40-44", "45-49"))
## no screen
df_g_NAT <- data.frame(lcmm_NAT$lambda_NAT$`20_24_2`)
df_lambda_NAT_2 <- df_g_NAT %>%
  mutate("20_24" = lambda,
         "25_29" = lcmm_NAT$lambda$`25_29_2`[, 2],
         "30_34" = lcmm_NAT$lambda$`30_34_2`[, 2],
         "35_39" = lcmm_NAT$lambda$`35_39_2`[, 2],
         "40_44" = lcmm_NAT$lambda$`40_44_2`[, 2],
         "45_49" = lcmm_NAT$lambda$`45_49_2`[, 2]) %>%
  select(-lambda) %>%
  gather("AgeGroup", "lambda", 2:7) %>%
  mutate(screen = 2)
df_lambda_NAT_2$AgeGroup <- factor(df_lambda_NAT_2$AgeGroup, levels = c("20_24", "25_29", "30_34", "35_39", "40_44", "45_49"),  
                             labels = c("20-24", "25-29", "30-34", "35-39", "40-44", "45-49"))
