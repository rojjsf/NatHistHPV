...
for(d in c(4)){
  ...
  for(f in c(35)) {
    ...
    for(i in c(1:4)){
      cat("\n ng: ", i) 
      if(i == 1) {
        lcmm_out$tr_lcmm[[stringr::str_c("tr_lcmm_", mina, "_", maxa, "ng_", i)]] <- 
        tr_lcmm <- lcmm(logRate ~ YsncS, random = ~ YsncS, subject = "Location",  ng = i, data = DpC)
        
        lcmm_out$tr_lcmm[[stringr::str_c("tr_lcmm_s_", mina, "_", maxa, "ng_", i)]] <- 
       ...
       # bic
      lcmm_out$tr_bic[[stringr::str_c("tr_bic_", mina, "_", maxa, "ng_", i)]] <- 
        bic <- round(tr_lcmm$BIC, 2)
      lcmm_out$tr_bic[[stringr::str_c("tr_bic_s_", mina, "_", maxa, "ng_", i)]] <- 
      ...
      lcmm_out$tr_conv[[stringr::str_c("tr_conv_", mina, "_", maxa, "ng_", i)]] <- tr_lcmm$conv
      lcmm_out$tr_conv_s[[stringr::str_c("tr_conv_s_", mina, "_", maxa, "ng_", i)]] <- tr_lcmm_s$conv
       
      # model coefficients
      #lcmm_out$tr_coef[[stringr::str_c("tr_coef_", mina, "_", maxa, "ng_", i)]] <- summary(tr_lcmm)
      #lcmm_out$tr_coef_s[[stringr::str_c("tr_coef_s_", mina, "_", maxa, "ng_", i)]] <- summary(tr_lcmm_s)
 ...
 # transition rate graphs (not in loop)
DpC_5y_grid <- gridExtra::grid.arrange(ggout$DpCplot20_24, ggout$DpCplot25_29, ggout$DpCplot30_34, ggout$DpCplot35_39)
DpC_10y_grid <- gridExtra::grid.arrange(ggout$DpCplot20_29, ggout$DpCplot25_34, ggout$DpCplot30_39, ggout$DpCplot35_44)
DpC_15y_grid <- gridExtra::grid.arrange(ggout$DpCplot20_34, ggout$DpCplot25_39, ggout$DpCplot30_44, ggout$DpCplot35_49)
