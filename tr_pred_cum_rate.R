...
## RUN: icc_statistical_transition_model.R
...
#### cum_dat: cum inc per age group and country ####
    # cumulative incidence as happend in whole fem. pop.

    #cum_dat <- Dp %>% 
    #  ungroup() %>%
    #  group_by(sgcentre, cid, Location, stY) %>% # also by YsncS as denominator has to be pop female of first year
    #  arrange(YsncS) %>%
    ...
    cum_dat <- Dp %>% 
      ...
      summarise(cumR = 1 - exp(-(last(cumsum(inc)))))
    
    # including mortality as competing risk
    #cum_dat <- Dp %>% 
    #  ungroup() %>%
    #  group_by(sgcentre, cid, Location, stY, YsncS) %>% 
    #  summarise(inc = last(cumsum(Nicc)/cumsum(PopFemale)),
...
#### Simulation to compare ci5 and Lexis cumulative incidence####
...
#### Lexis model ####            
      ...
      tr_pp$pp_all[[stringr::str_c("c_", c, "_", mina, "-", maxa)]] <- pp
      pp$time <- c(st:2012)
