---
title: "tr_results_2018"
author: "RSF"
date: "20 December 2018"
output: html_document
---
```{r echo = FALSE, eval = TRUE}
knitr::opts_chunk$set(cache = TRUE, echo = FALSE, message = FALSE, fig.show = TRUE, warning = FALSE, fig.width = 14, fig.height = 12)
```
# Age, time and the international correlation of cervical cancer    
...
For 15 Locations incidence and prevalence data were available in the same regions (Figure 1). If not, we included incidence rates from the most comparable registry in the same region or country. This concered Spain, China an Poland.   
The prevalence study in China, Shenzhen, was excluded as the rapidly changing demographic structure due to high migration rates makes incidence and prevalence estimates unreliable. In Uganda only women aged < 25 years were included in the study and could thus not be included in the analysis.  
...
##### Table 1: Most recent age-specific cervical cancer incidence and contemporary high-risk HPV prevalence
```{r echo = FALSE, results = 'asis', warning=FALSE, message=FALSE}
knitr::kable(corr_table[1:29, 1:11], caption = "Prevalence and incidence by location and age group",
             col.names = c("Location", "Study year","Prevalence 25-34 (%)", "Prevalence 35-44 (%)" , "Prevalence 45-54 (%)", "Prevalence 55-64 (%)", "Incidence source", 
                           "Incidence 25-34 (/100.000)","Incidence 35-44 (/100.000)" ,
                           "Incidence 45-54 (/100.000)", "Incidence 55-64 (/100.000)"))
```
...
gg_loc <- ggplot(upd_dat, aes(x = prev, y = IR)) +
  theme_classic() +
  ...
  facet_wrap(~age.grp, labeller = labeller(age.grp = labels), scales = "free_x") +
  theme(strip.text.x = element_text(hjust = 0.3, size = 20)) +
  labs(title = "International Correlation of hrHPV and Cervical Cancer",
       x = "hrHPV prevalence (%)",
       y = "Cervical Cancer Incidence (per 100 000 women)",
       color = "Location",
...
The estimated spearman rank coefficients (rho) and poisson coefficients (coef) are specified for the sccording age group.
The black line represents the fitted Poisson model and the grey area the 95% confidence interval *(fitted by the ggplot machinerie)*.   
...
## 2. Ecological estimate of the transition rate from HPV to cervical cancer  
...
Multiplying the population and the prevalence in each age group and location yields the corresponding number of hrHPV+ women. We then used the UN population data and CI5 incidence and UN mortality rates to estimate the number of new cancer cases and deaths per year in each age group and location. 
Assuming that the HPV prevalence is constant in each 10-year age group, thus that new infections and clearance even out, we could then consider the transition rate being the number of new cancer cases in the remaining event free HPV+ population for each age group, year and location.
...
We then fitted a latent class mixed model to describe the time trend of the transition rates since HPV detection, thus since the year of the equivalent HPV prevalence study.
Single-class quadratic I-splines with a random effect on intercept and slope were selected the best fitting model through the Bayesian Information Criterion included in the lcmm package in R.

To explore the effect of screening we included high versus unclear screening coverage as covariate in the latent class mixed model. There was no significant difference neither for 10-year nor for 5-year age groups. However a significant effect of age on the transition rate and its time trend could be observed when fitting one lcmm model for all transition rates and adjusting for age.   

```{r, eval = FALSE, echo = TRUE}
# adjusting for screening coverage
lcmm_out$tr_lcmm[[stringr::str_c(mina, "_", maxa)]] <-
      tr_lcmm <- lcmm(log_tr ~ YsncS * cov, random = ~ YsncS, subject = "Location",  ng = 1, data = DpC, link = "4-equi-splines")


# the effect of age on the time trand of the transition rate
tr_c <- lcmm(log_tr ~ YsncS * cov * ag, random = ~ YsncS, subject = "Location",  ng = 1, 
...
```
The number of knots in the link function was fixed to a maximum of 4 to assure sufficient data points in each knot.    
...
The model estimation and prediction were restricted to 14 years of follow up to assure a minimum of three locations.
```{r echo = TRUE, eval = FALSE}
# prediction
...
We then calculated the derivatives of the transition models in the two screening groups (Figure 6).
``` {r echo = TRUE, eval = FALSE}
#lambda
...
DpC_df_5$cov <- factor(DpC_df_5$cov, levels = c(1, 2), labels = c("yes", "no"))
...
##### Figure 4: Transition rate over time since high-risk HPV detection to cervical cancer by age and screening coverage estimated using ecological data. Stratification in 10-year age groups.
``` {r eval = TRUE, echo = FALSE}
#### 12. Transition Plots ####
...
ggplot(DpC_12, aes(x = YsncS, y = log_tr)) +
  theme_minimal() +
...
##### Figure 5: Transition rate over time since high-risk HPV detection to cervical cancer by age and screening coverage estimated using ecological data. Stratification in 5-year age groups.   
``` {r eval = TRUE, echo = FALSE}
ggplot(DpC_12_5, aes(x = YsncS, y = log_tr)) +
  theme_minimal() +
  geom_line(aes(col = Location, linetype = cov), size = 0.6) +
  scale_color_manual(values = col) +
  scale_linetype_manual(values = c(1, 3), labels = c("high screening", "low screening")) +
...
##### Figure 6: The change in cervical cancer transition rate over time since hpv detection by screening coverage 
```{r echo = FALSE, eval = TRUE}
