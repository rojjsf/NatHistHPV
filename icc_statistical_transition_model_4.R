...
#### 11.Plot data and best fit model ####

# one cowplot per age interval
leg <- cowplot::get_legend(ggout$tr_plot$`20_24`)
parr5y_1 <- cowplot::plot_grid(ggout$tr_plot$`20_24` + theme(legend.position = "none"), 
...
gg10y <- cowplot::plot_grid(parr10y, leg, rel_widths = c(3, .8))
 ...
 ## several classes
#m_logr <- data.frame("logr_1" = lcmm_out$tr_pred$tr_pred_s_25_34ng_2$pred[, 1], 
                    # "logr_lo_1" = lcmm_out$tr_pred$tr_pred_s_25_34ng_2$pred[, 3],
                    # "logr_hi_1" = lcmm_out$tr_pred$tr_pred_s_25_34ng_2$pred[, 5],
                    # "logr_2" = lcmm_out$tr_pred$tr_pred_s_25_34ng_2$pred[, 2], 
                    # "logr_lo_2" = lcmm_out$tr_pred$tr_pred_s_25_34ng_2$pred[, 4],
                    # "logr_hi_2" = lcmm_out$tr_pred$tr_pred_s_25_34ng_2$pred[, 6],
                    # "YsncS" = lcmm_out$tr_pred$tr_pred_s_25_34ng_2$times$YsncS)
#DpC <- DpCout$DpC25_34 %>%
#  full_join(., m_logr, by = "YsncS")
#mina <- 25
#maxa <- 34
...
#  ggplot2::ggplot(DpC, aes(x = YsncS, y = logRate)) + 
#  #geom_point(aes(color = Location)) + 
#  geom_ribbon(aes(x = YsncS, ymin = logr_lo_1, ymax = logr_hi_1), fill = 'azure3', alpha = 0.4)+
#  geom_ribbon(aes(x = YsncS, ymin = logr_lo_2, ymax = logr_hi_2), fill = 'cadetblue3', alpha = 0.4)+
#  geom_line(aes(color = Location), size = 0.75) +
#  scale_color_manual(values = col) +
#  ggtitle(stringr::str_c("cohort of HPV+ women ", mina, "-", maxa, "y (prev. intervals: ", ageintp, "y)")) +
#  ylab("log Incidence Rate in HPV+ women") +
#  xlab("Years since Prevalence Study") +
#  ylim(-11, -2) +
#  # geom_linerange(aes(x = YsncS, ymin = logRate - selog, ymax = logRate + selog, color = Location)) + # add standard error bars
#  theme_bw() + 
#  geom_line(aes(x = YsncS, y =  logr_1), col = "black", size = 1.3, linetype = 1) +
#  geom_line(aes(x = YsncS, y =  logr_2), col = "black", size = 1.3, linetype = 1) 
...
#### 12.   Plot all gamma #### 
  ...
  ggplot(df_gamma, aes(YsncS, gamma)) +
    ...
    labs(title = "Rate of change in cervical cancer incidence rate(ln)",
         subtitle = "per woman tested hrHPV+ at a given age",
         x = "Years since hrHPV detection", 
         y = "Rate of change in cervical cancer incidence rate(ln)",
    ...
    theme(legend.title = element_text(size=15, face="bold"),
          ...
          legend.key.size = unit(c(0.8, 0.8, 0.8, 1.3, 0.1), "cm"),
